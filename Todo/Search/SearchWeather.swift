...
struct SearchWeatherState: Equatable {
...
enum SearchWeatherAction: Equatable {
    case forecastResponse(Search.Result.ID, Result<Forecast, SearchWeatherClient.Failure>)
    case searchQueryChanged(String)
    case searchResponse(Result<Search, SearchWeatherClient.Failure>)
    case searchResultTapped(Search.Result)
}

struct SearchWeatherEnvironment {
    var weatherClient: SearchWeatherClient = SearchWeatherClient.live
    var mainQueue: AnySchedulerOf<DispatchQueue>
}

// MARK: - Search feature reducer
let searchWeatherReducer = Reducer<SearchWeatherState, SearchWeatherAction, SearchWeatherEnvironment> {
    ...
    case let .searchQueryChanged(query):
        ...
        return environment.weatherClient
            ...
            .catchToEffect(SearchWeatherAction.searchResponse)
...
struct SearchWeather: View {
    ...
    let store: Store<SearchWeatherState, SearchWeatherAction>
                ...
                HStack {
                    ...
                    TextField(
                        ...
                        text: viewStore.binding(
                            get: \.searchQuery, send: SearchWeatherAction.searchQueryChanged
                        )
                    )
    ...
    func weatherView(locationWeather: SearchWeatherState.Weather?) -> some View {
...
private func formattedWeatherDay(
    _ day: SearchWeatherState.Weather.Day, isToday: Bool
) -> String {
    let date =
    isToday
    ? "Today"
...
struct SearchWeather_Previews: PreviewProvider {
    static var previews: some View {
        let store = Store(
            initialState: SearchWeatherState(),
            reducer: searchWeatherReducer,
            environment: SearchWeatherEnvironment(
                weatherClient: SearchWeatherClient(
